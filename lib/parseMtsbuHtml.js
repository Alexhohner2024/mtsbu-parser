...
export function parseMtsbuHtml(html) {
  ...
  if (vehicleHeader.length) {
    ...
    if (!vehicleMake) {
      const mmHeadline = section
        .find(".headline")
        .filter((_, el) => textOf($(el)) === "Марка та модель")
        .first();
      if (mmHeadline.length) {
        const v = mmHeadline.next(".value").first();
        if (v.length) {
          const val = textOf(v);
          const split = val.split(/\s+/);
          vehicleMake = split[0] || null;
          vehicleModel = split.slice(1).join(" ") || null;
        }
      }
    }
